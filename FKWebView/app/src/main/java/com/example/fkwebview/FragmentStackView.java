...
public class FragmentStackView extends ViewGroup {
    ...
    public FragmentStackView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mTouchMarginSize = (int) (30 * density);//30dp
    ...
    public void push(Fragment fragment, String tag, boolean animate) {
        if(DEBUG) Log.i(TAG, "push() called with " + "fragment = [" + fragment + "], tag = [" + tag + "], animate = [" + animate + "]");
        if(isInAnimation()) {
     ...
     * @param animate 是否动画过程
     */
    public void replace(Fragment fragment, String tag, boolean animate) {
        if(DEBUG) Log.i(TAG, "replace() called with " + "fragment = [" + fragment + "], tag = [" + tag + "], animate = [" + animate + "]");
        if(isInAnimation()) {
            Log.w(TAG, "push isInAnimation");
            return;
        }
        if(fragmentStack.size() > 0) {
            waitReplaceFragment = fragmentStack.remove(fragmentStack.size() - 1);
        }
        try {
            fragmentManager.beginTransaction()
                    .add(getId(), fragment, tag)
    ...
    public void pop(boolean animate) {
        if(DEBUG) Log.i(TAG, "pop() called with " + "animate = [" + animate + "]");
        if(isInAnimation()) {
            Log.w(TAG, "pop isInAnimation");
            return;
        }
        int size = fragmentStack.size();
        if(size == 0 || getChildCount() == 0) {// TODO: 2015/8/24
            return;
        }
        if(animate) {
            forcedAnimation = true;
    ...
    public void popByIndex(int index, boolean include, boolean animate) {
        if(DEBUG) Log.i(TAG, "popByIndex() called with " + "index = [" + index + "], include = [" + include + "], animate = [" + animate + "]");
        if(isInAnimation()) {
    ...
    public Fragment peek() {
        ...
        return fragmentStack.get(size - 1);
    }
    ...
    public boolean isInAnimation() {
        if(DEBUG) Log.i(TAG, "isInAnimation forcedAnimation=" + forcedAnimation + " mDragState=" + mDragState);
    ...
    public float getBehindOffsetScale() {
        return behindOffsetScale;
    }

    public void setBehindOffsetScale(float behindOffsetScale) {
        if(behindOffsetScale < 0) {
            behindOffsetScale = 0;
        } else if(behindOffsetScale > 1) {
            behindOffsetScale = 1;
        }
        this.behindOffsetScale = behindOffsetScale;
    }
    ...
    private void dispatchOnPush() {
        if(DEBUG) Log.i(TAG, "dispatchOnPush() called with " + "");
        if(waitReplaceFragment != null) {
            try {
                fragmentManager.beginTransaction()
                        .remove(waitReplaceFragment)
                        .commitAllowingStateLoss();
                fragmentManager.executePendingTransactions();
            } catch(Exception ig) {
                Log.e(TAG, "dispatchOnPush", ig);
            }
            waitReplaceFragment = null;
        } else {
            int size = fragmentStack.size();
            if(size > 1) {
    ...
    private void dispatchOnPop() {
        if(DEBUG) Log.i(TAG, "dispatchOnPop() called with " + "");
        int size = fragmentStack.size();
        if(size > 0) {
    ...
    private void offsetActiveView(int dx) {
        ...
        //Log.i(TAG, "offsetActiveView() called with dx=" + dx + " aboveActiveView.getLeft()=" + aboveActiveView.getLeft() + " w=" + getWidth() + " aboveOffsetRatio=" + aboveOffsetRatio);
        ((LayoutParams) aboveActiveView.getLayoutParams()).offsetRatio = aboveOffsetRatio;
        setBehindActiveViewOffset(aboveOffsetRatio);
        invalidate();
    }
    ...
    private void smoothScroll(boolean open, int velocity) {
        ...
        if(velocity != 0) {
            ...
            duration = (int) ((distanceRatio + 1) * 215);
        }
    ...
    private void setDragState(int state) {
        if(DEBUG) Log.i(TAG, "setDragState mDragState=" + mDragState + " state=" + state);
        if(mDragState == state) {
            return;
        }
        mDragState = state;
        //enableSoftLayerType(state != STATE_IDLE);
        if(state == STATE_IDLE && aboveActiveView != null) {
            LayoutParams lp = (LayoutParams) aboveActiveView.getLayoutParams();
            if(DEBUG) Log.i(TAG, "setDragState STATE_IDLE offsetRatio=" + lp.offsetRatio);
    ...
    private void enableSoftLayerType(boolean enable) {
        if(isSoftLayerType == enable) {
            return;
        }
        isSoftLayerType = enable;
        int layerType = enable ? View.LAYER_TYPE_SOFTWARE : View.LAYER_TYPE_NONE;
        if(aboveActiveView != null) {
            aboveActiveView.setLayerType(layerType, null);
            aboveActiveView.setDrawingCacheEnabled(enable);
        }
        if(behindActiveView != null) {
            behindActiveView.setLayerType(layerType, null);
            behindActiveView.setDrawingCacheEnabled(enable);
        }
    }
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        return !forcedAnimation && super.dispatchTouchEvent(ev);
    }

    @Override
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        //if(DEBUG) Log.i(TAG, "onInterceptTouchEvent() called with " + "ev = [" + ev + "]");
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        //if(DEBUG) Log.d(TAG, "onTouchEvent() called with " + "ev = [" + ev + "]");
        ...
        switch(action) {
            ...
            case MotionEvent.ACTION_MOVE: {
                //Log.i(TAG, "ACTION_MOVE mActivePointerId=" + mActivePointerId);
                int activeIndex = MotionEventCompat.findPointerIndex(ev, mActivePointerId);
                if(activeIndex < 0) {
                    if(DEBUG) Log.w(TAG, "ACTION_MOVE pointerIndex < 0");
                    break;
    ...
    private void onSecondaryPointerDown(MotionEvent ev) {
        ...
        //Log.i(TAG, "onSecondaryPointerDown() called with " + "ev = [" + ev + "] mActivePointerId=" + mActivePointerId);
    }
    ...
    private void onSecondaryPointerUp(MotionEvent ev) {
        ...
        //Log.i(TAG, "onSecondaryPointerUp() called with " + "ev = [" + ev + "] mActivePointerId=" + MotionEventCompat.getPointerId(ev, pointerIndex) + " newmActivePointerId=" + mActivePointerId);
    }
